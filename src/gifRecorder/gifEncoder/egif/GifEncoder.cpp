...
static void getRasterBits(uint8_t *rasterBits, const uint8_t *pixels,
  ...
  for (int i = 0; i < nPixels; ++i) {
    rasterBits[i] =
        inxsearch(pixels[i * 3], pixels[i * 3 + 1], pixels[i * 3 + 2]);
  }
}
...
bool GifEncoder::push(PixelFormat format, const uint8_t *frame, int width,
...
bool GifEncoder::close() {
  ...
  if (m_useGlobalColorMap) {
    ...
    for (int i = 0; i < m_frameCount; ++i) {
      ...
      getRasterBits((uint8_t *)rasterBits, pixels,
                    m_frameWidth * m_frameHeight);

      encodeFrame(m_frameWidth, m_frameHeight, m_allFrameDelays[i], nullptr,
                  rasterBits);
    }
  }
...
void GifEncoder::encodeFrame(int width, int height, int delay, void *colorMap,
                             void *rasterBits) {
