...
int EGifCloseFile(GifFileType *GifFile, int *ErrorCode) {
  ...
  if (Private == NULL)
    ...
    /* This file was NOT open for writing: */
    if (ErrorCode != NULL) *ErrorCode = E_GIF_ERR_NOT_WRITEABLE;
    free(GifFile);
    return GIF_ERROR;
  } else {
    ...
    if (File && fclose(File) != 0) {
      if (ErrorCode != NULL) *ErrorCode = E_GIF_ERR_CLOSE_FAILED;
      free(GifFile);
      return GIF_ERROR;
    }

    free(GifFile);
    if (ErrorCode != NULL) *ErrorCode = E_GIF_SUCCEEDED;
  }
  return GIF_OK;
}
...
int EGifSpew(GifFileType *GifFileOut) {
  ...
  if (EGifCloseFile(GifFileOut, NULL) == GIF_ERROR) return (GIF_ERROR);
